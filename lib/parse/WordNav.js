...
(function (root, wNavFactory) {  // root is usually `window`
    ...
    */
        ...
        var positions 	= wNav._positions = [];
       	...
       	wNav.process = function ( sentenceArray, fragmentor ) {
       		if (!sentenceArray) { console.error('WordNav needs dataz to .process(). You gave it dis:', sentenceArray); }

            wNav.fragmentor = fragmentor;

            sentences = wNav._sentences = sentenceArray;  
            positions = wNav._positions = [];  // TODO: ??: Empty non-destructively??
