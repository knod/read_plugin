...
(function (root, splitFactory) {  // root is usually `window`
	...
	var WordSplitter = function ( charsNode, storage ) {
	...
	*/
		...
		var setMaxAtStart = function ( maxObj ) {
		/* 
		* Get settings established in this WordSplitter and set default max in storage if needed
		* Async
		*/
			if ( maxObj ) {
				rSpt.setMaxNumCharacters( maxObj.maxNumCharacters );
			} else {
				storage.set( defaults );
				rSpt.setMaxNumCharacters( defaults.maxNumCharacters );
			}
		}

		storage.get( 'maxNumCharacters', setMaxAtStart );



		// ============= USER INPUT ============= \\

		rSpt.setMaxNumCharacters = function ( max ) {

			var normalized = Math.max(max, 1);  // Minimum allowed length = 1

			storage.set( {'maxNumCharacters': max} );
			rSpt._settings.maxNumCharacters = max;
